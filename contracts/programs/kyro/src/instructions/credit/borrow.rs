...
pub fn handler(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    // Get dynamic credit limit from collateral (principal + earned interest)
    ...
    // Only set repayment_due_date on the FIRST borrow (C-01)
    let is_first_borrow =
    ...
    // Transfer from pool vault to borrower (PDA signing)
...
/// Update accrued interest on the credit line
fn update_interest(
    ...
    let new_interest = calculate_interest(credit_manager, credit_line, current_time);
    let new_accrued = (credit_line.interest_accrued as u128) + (new_interest as u128);
...
fn calculate_interest(
    ...
    // interest = borrowed * rate * time_elapsed / (BASIS_POINTS * SECONDS_PER_YEAR)
...
/// Get total collateral value (principal + pending interest from accumulator)
fn get_collateral_total(collateral_info: &CollateralInfo, accumulated: u128) -> u64 {
    let pending = calculate_pending_collateral_interest(collateral_info, accumulated);
...
fn calculate_pending_collateral_interest(
